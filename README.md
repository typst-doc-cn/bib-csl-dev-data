...
- `*.builtin.*`使用 Zotero 内置功能导出，`*.better.*`使用 Zotero 插件 [Better BibTeX for Zotero](https://retorque.re/zotero-better-bibtex/) 导出。
  ...
  - 若用 Better BibTeX 插件，则此栏会识别作弊语法，剩余内容默认清空。若修改[相应设置](https://retorque.re/zotero-better-bibtex/preferences/miscellaneous/#treat-the-extra-field-as-a-note)，则剩余内容会抄录到 BibLaTeX `annotation`字段或 CSL-JSON `note`字段。
  ...
  - 字段 | Fields
    ...
    - [将“其他”字段理解为note | Treat the `extra` field as a note](https://retorque.re/zotero-better-bibtex/preferences/miscellaneous/#treat-the-extra-field-as-a-note)
